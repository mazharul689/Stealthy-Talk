...
export class DecryptImageComponent implements OnInit {
  ...
  getImageData(id) {
    this.apiService.getAPI(`getstegoimagesbyimageid.php?image_id=${id}`).subscribe((data) => {
      let path: any = data
  ...
  //         binaryMsg += (data[i] & 1); // Extract least significant bit
  //       }

  //       let decryptedMsg = '';
  //       for (let i = 0; i < binaryMsg.length; i += 8) {
  //         const charCode = parseInt(binaryMsg.substr(i, 8), 2);
